...
#p=Pool(processes=7)
#results=[]
#results.append(p.apply_async(ombh2_prime,args=(pars,)))
#results.append(p.apply_async(omch2_prime,args=(pars,)))
#results.append(p.apply_async(thetastar_prime,args=(pars,)))
#results.append(p.apply_async(optical_depth_prime,args=(pars,)))
#results.append(p.apply_async(ns_prime,args=(pars,)))
#results.append(p.apply_async(As_prime,args=(pars,)))
#p.close()
#p.join()
#ombh2_CLprime=ombh2_obj.get()
#omch2_CLprime=omch2_obj.get()
#thetastar_CLprime=thetastar_obj.get()
#optical_depth_CLprime=optical_depth_obj.get()
#ns_CLprime=ns_obj.get()
#As_CLprime=As_obj.get()
#for res in results:
#    print(res.get())
#print(ombh2_CLprime)
q=Queue(7)
process_1=Process(target=ombh2_prime,args=(q,pars))
process_2=Process(target=omch2_prime,args=(q,pars))
process_3=Process(target=thetastar_prime,args=(q,pars))
process_4=Process(target=optical_depth_prime,args=(q,pars))
process_5=Process(target=ns_prime,args=(q,pars))
process_6=Process(target=As_prime,args=(q,pars))
...
#print(q.get())
#print(q.get())
newdict={**q.get(),**q.get(),**q.get(),**q.get(),**q.get(),**q.get()}
print(newdict)
process_1.terminate()
process_2.terminate()
process_3.terminate()
...
ombh2_CLprime=newdict['ombh2_CLprime']
omch2_CLprime=newdict['omch2_CLprime']
thetastar_CLprime=newdict['thetastar_CLprime']
optical_depth_CLprime=newdict['optical_depth_CLprime']
ns_CLprime=newdict['ns_CLprime']
As_CLprime=newdict['As_CLprime']

print('parameters time=',time.perf_counter()-time_0)
...
ls=np.arange(2501)
flag='all'
Noise=np.zeros((2501,4))
CMB_temperature=2.725#K
sensitivity_tem_143=1.5#muK/K
sensitivity_tem_217=3.3#muK/K
sensitivity_pol_143=3.0#muK/K
sensitivity_pol_217=6.6#muK/K
thetaFWHM_143=7*0.000291#rad
thetaFWHM_217=5*0.000291#rad
omega_minus_one_143_tem=(sensitivity_tem_143*CMB_temperature*thetaFWHM_143)**2
omega_minus_one_143_pol=(sensitivity_pol_143*CMB_temperature*thetaFWHM_143)**2
omega_minus_one_217_tem=(sensitivity_tem_217*CMB_temperature*thetaFWHM_217)**2
omega_minus_one_217_pol=(sensitivity_pol_217*CMB_temperature*thetaFWHM_217)**2
for i in np.arange(4):
    for l in np.arange(2501):
        if flag=='143Hz':
            if i==0:
                Noise[l,i]=omega_minus_one_143_tem*np.exp((l*(l+1)*(thetaFWHM_143)**2)/(8*np.log(2)))
            if i==1:
                Noise[l,i]=omega_minus_one_143_pol*np.exp((l*(l+1)*(thetaFWHM_143)**2)/(8*np.log(2)))
        if flag=='217Hz':
            if i==0:
                Noise[l,i]=omega_minus_one_217_tem*np.exp((l*(l+1)*(thetaFWHM_217)**2)/(8*np.log(2)))
            if i==1:
                Noise[l,i]=omega_minus_one_217_pol*np.exp((l*(l+1)*(thetaFWHM_217)**2)/(8*np.log(2)))
        if flag=='all':
            if i==0:
                Noise[l,i]=1/(1/(omega_minus_one_143_tem*np.exp((l*(l+1)*(thetaFWHM_143)**2)/(8*np.log(2))))+1/(omega_minus_one_217_tem*np.exp((l*(l+1)*(thetaFWHM_217)**2)/(8*np.log(2)))))
            if i==1:
                Noise[l,i]=1/(1/(omega_minus_one_143_pol*np.exp((l*(l+1)*(thetaFWHM_143)**2)/(8*np.log(2))))+1/(omega_minus_one_217_pol*np.exp((l*(l+1)*(thetaFWHM_217)**2)/(8*np.log(2)))))
...
time_0=time.perf_counter()
process_1=Process(target=get_TT_fisher_matrix,args=(Derivative,))
process_2=Process(target=get_EE_fisher_matrix,args=(Derivative,))
process_3=Process(target=get_TE_fisher_matrix,args=(Derivative,))
process_4=Process(target=get_combined_fisher_matrix,args=(Derivative,))
process_1.start()
process_2.start()
process_3.start()
process_4.start()
