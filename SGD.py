#trying to use SGD algorithm (some gradient lowering algerithm):changing the footstep until reaching some definite precision 1e-5 maybe
...
for i in np.arange(4):
    for l in np.arange(30,2501):
        ...
        print('best h is',0.05-(footstep)*(minimum_step[0][0]),'at l=',l,'on spectrum',i)
...
gamma=0.1#SGD with momentum algorithm https://zhuanlan.zhihu.com/p/32230623
while h>1e-8:
    ...
    for i in np.arange(4):#i is powerspectrum index
        for l in np.arange(30,2501):
            ...
            if index>=1:
                difference[l,i,index]=np.absolute((dp[l,i,index-1]-dp[l,i,index]))/(np.absolute(dp[l,i,index-1]))
                #print('difference=',difference[index])
                if difference[l,i,index-1]<1e-7:#there we treat '2' as a mark,telling us the difference following (<1e-4) should not be calculated
                    difference[l,i,index]=2
                if difference[l,i,index-1]==2:
                    difference[l,i,index]=2
    h-=gamma*footstep+(1-gamma)*difference
    print('loop=',index)
    index+=1
for i in np.arange(4):
    for l in np.arange(30,2501):
...
while h>0.001:
    ...
    for i in np.arange(4):#i is powerspectrum index
        for l in np.arange(30,2501):
            ...
            y=[y1_totCL[l,i],y2_totCL[l,i],totCL[l,i],y3_totCL[l,i],y4_totCL[l,i]]
            cs=CubicSpline(x,y,bc_type='natural')
            dp[l,i,index]=cs.derivative()(ns_0)
            #print('dp is',dp[index])
            if index>=1:
                difference[l,i,index]=np.absolute((dp[l,i,index-1]-dp[l,i,index]))/(np.absolute(dp[l,i,index-1]))
                #print('difference=',difference[index])
...
while h>0.00001:
    ...
    for i in np.arange(4):#i is powerspectrum index
        for l in np.arange(30,2501):
            x=[x1,x2,np.log(1e10*As_0),x3,x4]
            y=[y1_totCL[l,i],y2_totCL[l,i],totCL[l,i],y3_totCL[l,i],y4_totCL[l,i]]
            cs=CubicSpline(x,y,bc_type='natural')
            dp[l,i,index]=cs.derivative()(np.log(1e10*As_0))
            #print('dp is',dp[index])
            if index>=1:
                difference[l,i,index]=np.absolute((dp[l,i,index-1]-dp[l,i,index]))/(np.absolute(dp[l,i,index-1]))
                #print('difference=',difference[index])
